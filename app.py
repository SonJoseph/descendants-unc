...
@app.route('/api/createnode/name=<name>&relnWith=<relnWith>&relnId=<relnId>&relnType=<relnType>&spouseId=<spouseId>')
def createNode(name, relnWith, relnId, relnType, spouseId):
    global driver
    uid = uuid.uuid4().urn
    id = uid[9:]
    with driver.session() as session:
